...
namespace WebApplication1.Models
{
    ...
    public class ViewRecipeHandler : AuthorizationHandler<ViewRecipeRequirement, Recipe>
    {
            ...
            Recipe resource)
        {
            ...
            if (recipe.RecipeVisibility?.Name == RecipeVisibilityTypes.Private)
            {
            ...
            if (recipe.RecipeVisibility?.Name == RecipeVisibilityTypes.FriendsOnly)
            {
                if (recipe.UserId == currentUser.Id)
                {
                    context.Succeed(requirement);
                ...
                // Здесь нужно проверить, является ли currentUser другом владельца рецепта
